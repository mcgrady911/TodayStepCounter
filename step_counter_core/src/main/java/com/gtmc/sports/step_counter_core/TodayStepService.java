...
import com.gtmc.sports.step_counter_core.notification.api.compat.NotificationApiCompat;
...
//        Logger.i(TAG, "onCreate currStep :" + CURRENT_STEP);
        Map<String, String> map = getLogMap();
        map.put("current_step", String.valueOf(CURRENT_STEP));
        LoggerWraper.onEventInfo(this, StepCounterConstant.LOGGER_SERVICE_INITIALIZE_CURRSTEP, map);
    }
...
//        Logger.i(TAG, "onStartCommand CurrStep :" + CURRENT_STEP + "   mSeparate:" + mSeparate + "    mBoot:" + mBoot);
        mDbSaveCount = 0;
        ...
        LoggerWraper.onEventInfo(this, StepCounterConstant.LOGGER_SERVICE_ONSTARTCOMMAND, map);
...
//        Logger.i(TAG, "onBind : CurrStep : " + CURRENT_STEP);
        Map<String, String> map = getLogMap();
        map.put("current_step", String.valueOf(CURRENT_STEP));
        LoggerWraper.onEventInfo(this, StepCounterConstant.LOGGER_SERVICE_ONBIND, map);
...
//        sHandler.sendEmptyMessageDelayed(HANDLER_WHAT_REFRESH_NOTIFY_STEP, REFRESH_NOTIFY_STEP_DURATION);
    ...
    private void addStepCounterListener() {
        if (null != mStepCounter) {
            ...
            Map<String, String> map = getLogMap();
            map.put("current_step", String.valueOf(CURRENT_STEP));
            LoggerWraper.onEventInfo(this, StepCounterConstant.LOGGER_SERVICE_TYPE_STEP_COUNTER_HADREGISTER, map);
            return;
        }
        ...
        LoggerWraper.onEventInfo(this, StepCounterConstant.LOGGER_SERVICE_TYPE_STEP_COUNTER_REGISTER, map);
    }
...
//            Logger.i(TAG, "alreadly register TYPE_ACCELEROMETER : CurrStep : " + CURRENT_STEP);
            updateNotification(CURRENT_STEP);
            Map<String, String> map = getLogMap();
            map.put("current_step", String.valueOf(CURRENT_STEP));
            LoggerWraper.onEventInfo(this, StepCounterConstant.LOGGER_SERVICE_TYPE_ACCELEROMETER_HADREGISTER, map);
        ...
        map.put("current_step", String.valueOf(CURRENT_STEP));
        map.put("current_step_registerSuccess", String.valueOf(registerSuccess));
        LoggerWraper.onEventInfo(this, StepCounterConstant.LOGGER_SERVICE_TYPE_ACCELEROMETER_REGISTER, map);
    }

    @Override
    public void onDestroy() {
        LoggerWraper.onEventInfo(this, StepCounterConstant.LOGGER_TODAYSTEPSERVICE_ONDESTROY, "CURRENT_STEP=" + CURRENT_STEP);
        LoggerWraper.flush();
        super.onDestroy();
    }
    ...
    public boolean onUnbind(Intent intent) {
        LoggerWraper.onEventInfo(this, StepCounterConstant.LOGGER_TODAYSTEPSERVICE_ONUNBIND, "CURRENT_STEP=" + CURRENT_STEP);
    ...
    private void saveDb(boolean handler, int currentStep) {
        ...
        if (null != mTodayStepDBHelper) {
            if (!handler || !mTodayStepDBHelper.isExist(todayStepData)) {
                ...
                LoggerWraper.onEventInfo(this, StepCounterConstant.LOGGER_SERVICE_INSERT_DB, map);
            }
        }
    }
...
//        Logger.i(TAG, "cleanDb");
        Map<String, String> map = getLogMap();
        map.put("cleanDB_current_step", String.valueOf(CURRENT_STEP));
        LoggerWraper.onEventInfo(this, StepCounterConstant.LOGGER_SERVICE_CLEAN_DB, map);
        mDbSaveCount = 0;
    ...
    private String getTodayDate() {
        return StepUtils.getCurrentDate("yyyy-MM-dd");
    }
    ...
    private synchronized void updateNotification(int stepCount) {
        if (null != mNotificationApiCompat) {
            String km = StepUtils.getDistanceByStep(stepCount);
            String calorie = StepUtils.getCalorieByStep(stepCount);
            String contentText = calorie + " 千卡  " + km + " 公里";
    ...
    private final ISportStepInterface.Stub mIBinder = new ISportStepInterface.Stub() {
        ...
        private JSONArray getSportStepJsonArray(List<TodayStepData> todayStepDataArrayList) {
            return StepUtils.getSportStepJsonArray(todayStepDataArrayList);
        }
    ...
    private void getSensorRate() {
        ...
        try {
            ...
            LoggerWraper.onEventInfo(this, StepCounterConstant.LOGGER_SERVICE_SENSORRATE_INVOKE, map);
