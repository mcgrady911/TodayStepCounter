...
import static com.gtmc.sports.step_counter_core.StepCounterConstant.CONTINUOUS_STEP_COUNT_STAY_TIME;
        ...
        public boolean handleMessage(Message msg) {
            switch (msg.what) {
                case HANDLER_WHAT_TEST_LOGGER: {
                    ...
                    LoggerWraper.onEventInfo(mContext, StepCounterConstant.LOGGER_TYPE_ACCELEROMETER_TIMER, map);
//                    Logger.i(TAG,"onSensorChanged mCount : " +  mCount +
//                            "   this.count :" + count +
//                            "   SensorCount : " + mLoggerSensorCount);
    ...
    public TodayStepDetector(Context context, OnStepCounterListener onStepCounterListener) {
        ...
        LoggerWraper.onEventInfo(mContext, StepCounterConstant.LOGGER_TYPE_ACCELEROMETER_CONSTRUCTOR, map);
//        Logger.i(TAG, "TodayStepDetector mCount : " + mCount +
//                "   mTodayDate:" + mTodayDate);
    ...
    private synchronized void dateChangeCleanStep() {
        ...
        if (!getTodayDate().equals(mTodayDate)) {
            ...
            setSteps(0);
            LoggerWraper.onEventInfo(mContext, StepCounterConstant.LOGGER_TYPE_ACCELEROMETER_DATECHANGECLEANSTEP);
    ...
    private void countStep() {
        ...
        if (this.timeOfThisPeak1 - this.timeOfLastPeak1 <= CONTINUOUS_STEP_COUNT_STAY_TIME) {
            if (this.count < StepCounterConstant.EVOKE_SENSOR_STEP_COUNT) {
                this.count++;
            } else if (this.count == StepCounterConstant.EVOKE_SENSOR_STEP_COUNT) {
                this.count++;
