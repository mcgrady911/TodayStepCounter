...
class TodayStepDBHelper extends SQLiteOpenHelper implements ITodayStepDBHelper {
    ...
    public synchronized TodayStepData getMaxStepByDate(long millis) {
        Cursor cursor = getReadableDatabase().rawQuery(SQL_QUERY_STEP_ORDER_BY, new String[]{StepUtils.dateFormat(millis, "yyyy-MM-dd")});
    ...
    public synchronized List<TodayStepData> getStepListByStartDateAndDays(String startDate, int days) {
        ...
        for (int i = 0; i < days; i++) {
            ...
            calendar.setTimeInMillis(StepUtils.getDateMillis(startDate, DATE_PATTERN_YYYY_MM_DD));
            calendar.add(Calendar.DAY_OF_YEAR, i);
            Cursor cursor = getReadableDatabase().rawQuery(SQL_QUERY_STEP_BY_DATE,
                    new String[]{StepUtils.dateFormat(calendar.getTimeInMillis(), DATE_PATTERN_YYYY_MM_DD)});
    ...
    public synchronized void clearCapacity(String curDate, int limit) {
        ...
        try {
            Calendar calendar = Calendar.getInstance();
            calendar.setTimeInMillis(StepUtils.getDateMillis(curDate, DATE_PATTERN_YYYY_MM_DD));
            calendar.add(Calendar.DAY_OF_YEAR, -(mLimit));
            String date = StepUtils.dateFormat(calendar.getTimeInMillis(), DATE_PATTERN_YYYY_MM_DD);
            Log.e(TAG, date);
            ...
            for (TodayStepData tmpTodayStepData : todayStepDataList) {
                long dbTodayDate = StepUtils.getDateMillis(tmpTodayStepData.getToday(), DATE_PATTERN_YYYY_MM_DD);
