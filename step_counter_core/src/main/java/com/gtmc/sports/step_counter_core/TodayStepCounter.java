...
import static com.gtmc.sports.step_counter_core.StepCounterConstant.HANDLER_WHAT_TEST_LOGGER;
import static com.gtmc.sports.step_counter_core.StepCounterConstant.WHAT_TEST_LOGGER_DURATION;
...
class TodayStepCounter implements ITodayStepCounter, SensorEventListener {
    ...
    private float mLoggerSensorStep = 0f;
    private int mLoggerCounterStep = 0;
    private int mLoggerCurrStep = 0;
    private int mLoggerOffsetStep = 0;
    /**
     * 传感器回调次数
     */
    private long mLoggerSensorCount = 0;
        ...
        public boolean handleMessage(Message msg) {
            switch (msg.what) {
                case HANDLER_WHAT_TEST_LOGGER: {
                    ...
                    map.put("sCurrStep", String.valueOf(mLoggerCurrStep));
                    map.put("counterStep", String.valueOf(mLoggerCounterStep));
                    map.put("SensorStep", String.valueOf(mLoggerSensorStep));
                    map.put("sOffsetStep", String.valueOf(mLoggerOffsetStep));
                    map.put("SensorCount", String.valueOf(mLoggerSensorCount));
                    //增加电量、息屏状态
                    ...
                    LoggerWraper.onEventInfo(mContext, StepCounterConstant.LOGGER_TYPE_STEP_COUNTER_TIMER, map);
    ...
    public TodayStepCounter(Context context, OnStepCounterListener onStepCounterListener, boolean separate, boolean boot) {
        ...
        LoggerWraper.onEventInfo(mContext, StepCounterConstant.LOGGER_TYPE_STEP_CONSTRUCTOR, map);
    ...
    public void unregisterBroadcastReceiver() {
        if (mBatInfoReceiver != null && mContext != null) {
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (event.sensor.getType() == Sensor.TYPE_STEP_COUNTER) {
            ...
            if (mCleanStep) {
                ...
                LoggerWraper.onEventInfo(mContext, StepCounterConstant.LOGGER_TYPE_STEP_CLEANSTEP, map);
            } else {
                //处理关机启动
                if (mShutdown || shutdownByCounterStep(counterStep)) {
                    ...
                    LoggerWraper.onEventInfo(mContext, StepCounterConstant.LOGGER_TYPE_STEP_SHUTDOWN, map);
                }
            }
            ...
            if (sCurrStep < 0) {
                ...
                LoggerWraper.onEventInfo(mContext, StepCounterConstant.LOGGER_TYPE_STEP_TOLERANCE, map);
            }
            ...
            mLoggerSensorStep = event.values[0];
            mLoggerCounterStep = counterStep;
            mLoggerCurrStep = sCurrStep;
            mLoggerOffsetStep = sOffsetStep;
            updateStepCounter();
            if (mLoggerSensorCount == 0) {
                sHandler.removeMessages(HANDLER_WHAT_TEST_LOGGER);
    ...
    private boolean shutdownByCounterStep(int counterStep) {
        if (mCounterStepReset) {
            ...
            if (counterStep < PreferencesHelper.getLastSensorStep(mContext)) {
                LoggerWraper.onEventInfo(mContext, StepCounterConstant.LOGGER_TYPE_STEP_SHUTDOWNBYCOUNTERSTEP, "当前传感器步数小于上次传感器步数");
...
//                Logger.e(TAG, "当前传感器步数小于上次传感器步数肯定是重新启动了，只是用来增加精度不是绝对的");
    ...
    private boolean shutdownBySystemRunningTime() {
        if (PreferencesHelper.getElapsedRealtime(mContext) > SystemClock.elapsedRealtime()) {
            LoggerWraper.onEventInfo(mContext, StepCounterConstant.LOGGER_TYPE_STEP_SHUTDOWNBYSYSTEMRUNNINGTIME, "本地记录的时间，判断进行了关机操作");
...
//            Logger.e(TAG, "上次运行的时间大于当前运行时间判断为重启，只是增加精度，极端情况下连续重启，会判断不出来");
    ...
    private synchronized void dateChangeCleanStep() {
        ...
        if (!getTodayDate().equals(mTodayDate) || mSeparate) {
            ...
            LoggerWraper.onEventInfo(mContext, StepCounterConstant.LOGGER_TYPE_STEP_COUNTER_DATECHANGECLEANSTEP, map);
            WakeLockUtils.getLock(mContext);
