...
final class PreferencesHelper {
    ...
    private static final Map<String, PreferencesHelper> SP_UTILS_MAP = new HashMap<>();

    private SharedPreferences sp;

    /**
     * Return the single {@link PreferencesHelper} instance
     *
     * @return the single {@link PreferencesHelper} instance
     */
    private static PreferencesHelper getInstance(Context context) {
        return getInstance(context, APP_SHARD, Context.MODE_PRIVATE);
    }
     ...
     * @return the single {@link PreferencesHelper} instance
     */
    private static PreferencesHelper getInstance(Context context, final int mode) {
        return getInstance(context,APP_SHARD, mode);
    }
     ...
     * @return the single {@link PreferencesHelper} instance
     */
    private static PreferencesHelper getInstance(Context context, String spName) {
     ...
     * @return the single {@link PreferencesHelper} instance
     */
    private static PreferencesHelper getInstance(Context context, String spName, final int mode) {
        if (isSpace(spName)) spName = "PreferencesHelper";
        PreferencesHelper PreferencesHelper = SP_UTILS_MAP.get(spName);
        if (PreferencesHelper == null) {
            synchronized (PreferencesHelper.class) {
                PreferencesHelper = SP_UTILS_MAP.get(spName);
                if (PreferencesHelper == null) {
                    PreferencesHelper = new PreferencesHelper(context, spName, mode);
                    SP_UTILS_MAP.put(spName, PreferencesHelper);
                }
            }
        }
    ...
    private PreferencesHelper(Context context, final String spName) {
        sp = context.getSharedPreferences(spName, Context.MODE_PRIVATE);
    }

    private PreferencesHelper(Context context, final String spName, final int mode) {
