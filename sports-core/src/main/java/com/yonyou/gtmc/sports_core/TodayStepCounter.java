...
import static com.yonyou.gtmc.sports_core.ConstantDef.HANDLER_WHAT_TEST_LOGGER;
import static com.yonyou.gtmc.sports_core.ConstantDef.WHAT_TEST_LOGGER_DURATION;
...
class TodayStepCounter implements ITodayStepCounter, SensorEventListener{
        ...
        public boolean handleMessage(Message msg) {
            switch (msg.what) {
                case HANDLER_WHAT_TEST_LOGGER: {
                    ...
                    LoggerWraper.onEventInfo(LoggerConstant.TYPE_STEP_COUNTER_TIMER, map);
    ...
    public TodayStepCounter(Context context, OnStepCounterListener onStepCounterListener, String userId, boolean separate, boolean boot) {
        ...
        sCurrStep = (int) TodayStepPreferencesHelper.getCurrentStep(context);
        cleanStep = TodayStepPreferencesHelper.getCleanStep(context);
        todayDate = TodayStepPreferencesHelper.getStepToday(context);
        sOffsetStep = (int) TodayStepPreferencesHelper.getStepOffset(context);
        shutdown = TodayStepPreferencesHelper .getShutdown(context);
        //开机启动监听到，一定是关机开机了
        boolean isShutdown = shutdownBySystemRunningTime();
        if (this.boot || isShutdown) {
            shutdown = true;
            TodayStepPreferencesHelper.setShutdown(context, shutdown);
        }
        HashMap<String, String> map = new HashMap<>();
        ...
        map.put("lastSensorStep", String.valueOf(TodayStepPreferencesHelper.getLastSensorStep(context)));
        LoggerWraper.onEventInfo(LoggerConstant.TYPE_STEP_CONSTRUCTOR, map);
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (event.sensor.getType() == Sensor.TYPE_STEP_COUNTER) {
            ...
            if (cleanStep) {
                ...
                LoggerWraper.onEventInfo(LoggerConstant.TYPE_STEP_CLEANSTEP, map);
            } else {
                //处理关机启动
                if (shutdown || shutdownByCounterStep(counterStep)) {
                    ...
                    LoggerWraper.onEventInfo(LoggerConstant.TYPE_STEP_SHUTDOWN, map);
                }
            }
            ...
            if (sCurrStep < 0) {
                ...
                LoggerWraper.onEventInfo(LoggerConstant.TYPE_STEP_TOLERANCE, map);
            }

            TodayStepPreferencesHelper.setCurrentStep(context, sCurrStep);
            TodayStepPreferencesHelper.setElapsedRealtime(context, SystemClock.elapsedRealtime());
            TodayStepPreferencesHelper.setLastSensorStep(context, counterStep);
    ...
    private void cleanStep(int counterStep) {
        ...
        TodayStepPreferencesHelper.setStepOffset(context, userId, sOffsetStep);

        cleanStep = false;
        TodayStepPreferencesHelper.setCleanStep(context, userId, cleanStep);
        loggerCurrentStep = sCurrStep;
        loggerOffsetStep = sOffsetStep;
    }

    private void shutdown(int counterStep) {
        int tmpCurrStep = (int) TodayStepPreferencesHelper.getCurrentStep(context, userId);
        //重新设置offset
...
//        sOffsetStep = -tmpCurrStep;
    ...
    private boolean shutdownByCounterStep(int counterStep) {
        if (counterStepReset) {
            ...
            if (counterStep < TodayStepPreferencesHelper.getLastSensorStep(context)) {
                LoggerWraper.onEventInfo(LoggerConstant.TYPE_STEP_SHUTDOWNBYCOUNTERSTEP, "当前传感器步数小于上次传感器步数");
...
//                Logger.e(TAG, "当前传感器步数小于上次传感器步数肯定是重新启动了，只是用来增加精度不是绝对的");
    ...
    private boolean shutdownBySystemRunningTime() {
        if (TodayStepPreferencesHelper.getElapsedRealtime(context, userId) > SystemClock.elapsedRealtime()) {
            LoggerWraper.onEventInfo(LoggerConstant.TYPE_STEP_SHUTDOWNBYSYSTEMRUNNINGTIME, "本地记录的时间，判断进行了关机操作");
...
//            Logger.e(TAG, "上次运行的时间大于当前运行时间判断为重启，只是增加精度，极端情况下连续重启，会判断不出来");
    ...
    private synchronized void dateChangeCleanStep() {
        ...
        if (!getTodayDate().equals(todayDate) || separate) {
            ...
            LoggerWraper.onEventInfo(LoggerConstant.TYPE_STEP_COUNTER_DATECHANGECLEANSTEP, map);
            WakeLockUtils.getLock(context);

            cleanStep = true;
            TodayStepPreferencesHelper.setCleanStep(context, cleanStep);

            todayDate = getTodayDate();
            TodayStepPreferencesHelper.setStepToday(context, todayDate);
    ...
    public int getCurrentStep() {
        sCurrStep = (int) TodayStepPreferencesHelper.getCurrentStep(context, userId);
        return sCurrStep;
    }
