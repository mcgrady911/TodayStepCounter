...
public class TodayStepDetector implements ITodayStepCounter, SensorEventListener {
        ...
        public boolean handleMessage(Message msg) {
            switch (msg.what) {
                case HANDLER_WHAT_TEST_LOGGER: {
                    ...
                    LoggerWraper.onEventInfo(LoggerConstant.TYPE_ACCELEROMETER_TIMER, map);
    ...
    public TodayStepDetector(Context context, OnStepCounterListener onStepCounterListener, String userId) {
        super();
        mContext = context;
        ...
        mCount = (int) TodayStepPreferencesHelper.getCurrentStep(mContext, userId);
        mTodayDate = TodayStepPreferencesHelper.getStepToday(mContext, userId);
        Map<String, String> map = new HashMap<>();
        map.put("mCount", String.valueOf(mCount));
        map.put("mTodayDate", String.valueOf(mTodayDate));
        LoggerWraper.onEventInfo(LoggerConstant.TYPE_ACCELEROMETER_CONSTRUCTOR, map);
    ...
    private synchronized void dateChangeCleanStep() {
        ...
        if (!getTodayDate().equals(mTodayDate)) {
            ...
            mCount = 0;
            TodayStepPreferencesHelper.setCurrentStep(mContext, userId, mCount);

            mTodayDate = getTodayDate();
            TodayStepPreferencesHelper.setStepToday(mContext, userId, mTodayDate);

            setSteps(0);
            LoggerWraper.onEventInfo(LoggerConstant.TYPE_ACCELEROMETER_DATECHANGECLEANSTEP);
